...
@section('content')
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">{{ __('Edycja planu działań') }}</div>

                    <div class="card-body">
                        {{ Breadcrumbs::render('clubActionPlan.edit', $club, $report) }}
                        <form method="POST" action="{{ route('clubActionPlan.update',[$club,$report]) }}">
                            @method('PATCH')
                            @csrf
                            <div class="container mt-4 mb-4">
                                <div class="row justify-content-md-center">
                                    <div class="col-md-12 col-lg-8">
                                        <h1 class="h2 mb-4">{{ __('Opis planowanych działań') }}</h1>
                            ...
                            <a href="{{ route('clubActionPlan.generatePDF', [$club, $report])}}"
                                   class="btn btn-primary">Wygeneruj sprawozdanie (PDF)</a>
                            <a href="{{ route('clubActionPlan.generateDoc', [$club, $report])}}"
                        ...
                        @if($club->getLoggedUserRoleName() == 'opiekun_koła' || $club->getLoggedUserRoleName() == 'przewodniczący_koła')
                            ...
                            @if($attachments_send == TRUE)
                                ...
                                @foreach($report->attachments as $attachment)
                                    <a href="{{ route('downloadAttachment', ['path' => $attachment->name])}}">{{$attachment->original_file_name}}</a><br>
                                @endforeach
                                <form method="POST"
                                      action="{{ route('clubReport.submitToSupervisor',[$club,$report]) }}"
